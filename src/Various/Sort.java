...
public class Sort {
	...
	public static void run_experiments(int experiments, int[] array_sizes)
	{
		...
		for(int m = 0; m < array_sizes.length; m++)
		{
			...
			int[] array_data2 = new int[array_data.length];
			Arrays.sort(array_data);
			int Nu = 0;
			for (int i = array_data.length-1; i >= 0; i--) {
				 array_data2[i] = array_data[Nu];
				 Nu++;
				 }
			//System.out.println(Arrays.toString(array_data2));
			
			for(int i = 0; i < experiments; i++)
			{
				time_start = System.nanoTime();
				//BubbleSort(array_data2);
				//QuickSort(array_data, array_data.length);
				QuickSort_2(array_data2, 0 ,array_data2.length - 1);
				//MergeSort(array_data);
				time_end = System.nanoTime();

				time_results[i] = ((time_end - time_start));
			}
			for (int i = 0; i < time_results.length; i++) {
				System.out.println(time_results[i]);
				 }
