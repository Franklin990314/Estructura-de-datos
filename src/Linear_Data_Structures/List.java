...
public class List {
	...
	{
		Node temp = head; //It create a temporary of node
		Node previous = temp; //It create a node previous
		
		for(int i = 0; i < index; i++) //Search the position the node to insert
		{
			previous = temp; //The previous node it is pointed to the new node
			temp = temp.getNext(); // Go to next node
		}
		
		newNode.setNext(previous.getNext()); //Point the new node to the next node of the previous
	...
	public void deleteAtEnd()
	{
		...
		while(toDelete.getNext() != null) //while the node do not this empty go to next node
		{
	...
	public Node get(int index)
	{
		...
		return result; //Return the node to get
	}
